...
class IMipPlugin extends SabreIMipPlugin {
	...
	public function schedule(Message $iTipMessage) {
		...
		$lastOccurrence = $this->imipEmailService->getLastOccurrence($iTipMessage->message);
		...
		$attendee = $this->imipEmailService->getCurrentAttendee($iTipMessage);
		$partstat = null;
		$this->imipEmailService->setL10n($attendee);
		...
		switch (strtolower($iTipMessage->method)) {
			...
			case self::METHOD_CANCEL:
				...
				$data = $this->imipEmailService->buildCancelledBodyData($vEvent);
				break;
			default:
		...
		if (strcasecmp($method, self::METHOD_REQUEST) === 0 && $this->imipEmailService->getAttendeeRsvpOrReqForParticipant($attendee)) {
			...
			*/
			...
			if (strcmp('yes', $invitationLinkRecipients[0]) === 0
				...
				$token = $this->imipEmailService->createInvitationToken($iTipMessage, $vEvent, $lastOccurrence);
				$this->imipEmailService->addResponseButtons($template, $token);
				$this->imipEmailService->addMoreOptionsButton($template, $token);
			}
		}
		...
		$vCalendar = $this->imipEmailService->generateVCalendar($iTipMessage, $vEvent);
