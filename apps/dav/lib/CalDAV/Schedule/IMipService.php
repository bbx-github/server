...
class IMipService {
	...
	public function buildBodyData(VEvent $vEvent, ?VEvent $oldVEvent): array {
		...
		$data = [
			'meeting_when' => $newMeetingWhen,
			'meeting_description' => $newDescription,
			'meeting_title' => $newSummary,
			'meeting_location' => $newLocation,
			'meeting_url' => isset($vEvent->URL) ? (string)$vEvent->URL : '',
			'meeting_url_html' => $newUrl
		];

		if(!empty($oldVEvent)) {
			$oldMeetingWhen = $this->generateWhenString($oldVEvent);
			$oldSummary 	= isset($oldVEvent->SUMMARY) && (string)$oldVEvent->SUMMARY !== '' ? (string)$oldVEvent->SUMMARY : $this->l10n->t('Untitled event');;
			$oldDescription = isset($oldVEvent->DESCRIPTION) && (string)$oldVEvent->DESCRIPTION !== '' ? (string)$oldVEvent->DESCRIPTION : $defaultVal;
			$oldUrl         = isset($oldVEvent->URL) && (string)$oldVEvent->URL !== '' ? sprintf('<a href="%1$s">%1$s</a>', $oldVEvent->URL) : $defaultVal;
			...
			$data['meeting_when_html']        = ($oldMeetingWhen !== $newMeetingWhen && $oldMeetingWhen !== null) ? sprintf($strikethrough, $oldMeetingWhen, $newMeetingWhen) : $newMeetingWhen;
			$data['meeting_title_html']       = ($oldSummary !== $newSummary) ? sprintf($strikethrough, $oldSummary, $newSummary) : $newSummary;
			$data['meeting_description_html'] = ($oldDescription !== $newDescription) ? sprintf($strikethrough, $oldDescription, $newDescription) : $newDescription;
			$data['meeting_url_html']         = ($oldUrl !== $newUrl) ? sprintf($strikethrough, $oldUrl, $newUrl) : $newUrl;
			$data['meeting_location_html']    = ($oldLocation !== $newLocation) ? sprintf($strikethrough, $oldLocation, $newLocation) : $newLocation;
		}
		return $data;
	}
	...
	public function buildCancelledBodyData(VEvent $vEvent): array {
		...
		$data['meeting_title'] = $newSummary !== '' ?: $this->l10n->t('Untitled event');
		...
		if ($method === IMipPlugin::METHOD_CANCEL) {
			...
			switch ($partstat) {
				...
				case 'DECLINED':
					...
					break;
				default:
					$partstat = $this->l10n->t('%1$s has responded your invitation', [$sender]);
					break;
			}
